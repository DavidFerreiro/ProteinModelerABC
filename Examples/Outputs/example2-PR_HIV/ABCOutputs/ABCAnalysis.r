...
#Path
address<-paste("/mnt/netapp2/Store_uni/home/uvi/be/dfg/New_Analysis/example2-PR_HIV/ABCOutputs", sep="")
setwd(address)
############################
...
pdf(figureName1)
try(plot(cv.modsel, names.arg=c("Fitness", "HIVb", "Neutral")), silent=TRUE)
try(legend(x="topright", inset = c(-0.05, -0.16), legend = c("Fitness", "HIVb", "Neutral"), xpd = TRUE, bty = "n", fill = c('grey25', 'grey50', 'grey75')), silent=TRUE)
invisible(dev.off())
############################
...
res.gfit.HIVb=suppressWarnings(gfit(target=FullRealmatrix, sumstat=FullSSmatrix[ModelsVector=="HIVb",], statistic=mean, nb.replicate=100))
write(paste("   -HIVb:",sep=""),"Results_text.txt",append=T)
capture.output(summary(res.gfit.HIVb), file = "Results_text.txt", append = TRUE)
plot(res.gfit.HIVb, main="Histogram under HIVb")
