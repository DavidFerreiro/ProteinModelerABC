...
#Path
address<-paste("/mnt/netapp2/Store_uni/home/uvi/be/dfg/UPLOAD/example3-GAG_HIV/ABCOutputs", sep="")
setwd(address)
############################
...
pdf(figureName1)
try(plot(cv.modsel, names.arg=c("Fitness", "Neutral", "RtRev")), silent=TRUE)
try(legend(x="topright", inset = c(-0.05, -0.16), legend = c("Fitness", "Neutral", "RtRev"), xpd = TRUE, bty = "n", fill = c('grey25', 'grey50', 'grey75')), silent=TRUE)
invisible(dev.off())
############################
...
res.gfit.RtRev=suppressWarnings(gfit(target=FullRealmatrix, sumstat=FullSSmatrix[ModelsVector=="RtRev",], statistic=mean, nb.replicate=100))
write(paste("   -RtRev:",sep=""),"Results_text.txt",append=T)
capture.output(summary(res.gfit.RtRev), file = "Results_text.txt", append = TRUE)
plot(res.gfit.RtRev, main="Histogram under RtRev")
invisible(dev.off())
