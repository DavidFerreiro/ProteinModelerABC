...
if __name__ == "__main__":
    ...
    for g in range(0, bin.Scripts.Variables.LongEM):
        with open('ProteinModelerABC.out', 'a') as Error:
        ...
        result = e_pool.map(bin.Scripts.Functions.E_Simu, range(int(bin.Scripts.Variables.Total_E_simu)), chunksize=1)
    ...
    if bin.Scripts.Variables.LongSM == 0:
        pass
    elif bin.Scripts.Variables.LongSM == 1:
        FI = 'Fitness' in bin.Scripts.Variables.SubstitutionModel
        NI = 'Neutral' in bin.Scripts.Variables.SubstitutionModel
        if FI == True:
            with open('ProteinModelerABC.out', 'a') as Error:
            ...
            o_pool = multiprocessing.Pool(int(bin.Scripts.Variables.NumberOfProcessors))

            result = o_pool.map(bin.Scripts.Functions.S_Simu_1, range(int(bin.Scripts.Variables.NumberOfSimulations)), chunksize=1)
    ...
    elif bin.Scripts.Variables.LongSM == 2:
        with open('ProteinModelerABC.out', 'a') as Error:
            Error.write('Starting simulations based on Fitness amino acid substitution model\n')
            Error.close()
        print('Starting simulations based on Fitness amino acid substitution model')
        ...
        result = o_pool.map(bin.Scripts.Functions.S_Simu_F, range(int(bin.Scripts.Variables.NumberOfSimulations)), chunksize=1)
        ...
        result = o_pool.map(bin.Scripts.Functions.S_Simu_N, range(int(bin.Scripts.Variables.NumberOfSimulations)), chunksize=1)
    ...
    bin.Scripts.Functions.CheckSimu()

    bin.Scripts.Functions.RealSummarySta(bin.Scripts.Variables.NameOfPhylipFile)
    ...
    result = a_pool.map(bin.Scripts.Functions.SimuSummarySta, range(int(bin.Scripts.Variables.Total_simu)), chunksize=1)
