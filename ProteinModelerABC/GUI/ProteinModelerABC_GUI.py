...
def main_window():
    ...
    def Hide_SM_Settings():
        ...
        if is_oon:
            ...
            is_oon = False
            lab28.grid(row=80, column=1, sticky='W', padx=5)
            lab28_2.grid(row=81, column=1, sticky='W', padx=10)
            lab28_3.grid(row=82, column=1, sticky='W', padx=10)
            G_menu.grid(row=83, column=1, sticky='W', padx=10, pady=5)
            G_menu_1.grid(row=83, column=1, sticky='W', padx=140, pady=5)
            lab28_4.grid(row=82, column=1, sticky='W', padx=285)
            G_menu_2.grid(row=83, column=1, sticky='W', padx=285, pady=5)
            lab28_5.grid(row=82, column=1, sticky='W', padx=360)
            G_menu_3.grid(row=83, column=1, sticky='W', padx=360, pady=5)
            lab29.grid(row=84, column=1, sticky='W', padx=5)
            lab29_2.grid(row=85, column=1, sticky='W', padx=10)
            I_menu.grid(row=87, column=1, sticky='W', padx=10, pady=5)
            lab29_3.grid(row=86, column=1, sticky='W', padx=10)
            I_menu_1.grid(row=87, column=1, sticky='W', padx=140, pady=5)
            lab29_4.grid(row=86, column=1, sticky='W', padx=285)
            I_menu_2.grid(row=87, column=1, sticky='W', padx=285, pady=5)
            lab29_5.grid(row=86, column=1, sticky='W', padx=360)
            I_menu_3.grid(row=87, column=1, sticky='W', padx=360, pady=5)
            updateScrollRegion()
        else:
            Additional_parameters.config(text="+ Optional parameters")
            is_oon = True
    ...
    def save_selected_values():
        ...
        file.write('########################################\n\n')
        file.write('# Target alignment file. Phylip format, see documentation for details -- MANDATORY --\n')
        ...
        file.write('# PDB protein structure. PDB protein structure used to structural substitution models simulations and to calculate proteins free energy -- MANDATORY --\n')
        file.write('*Template=' + str(Template_directory) + '\n\n')
        file.write('# Chain of the PDB protein structure used. See documentation for details -- MANDATORY --\n')
        file.write('*Chain=' + ChainEntry.get() + '\n\n')
        file.write('# Show running information (simulations and summary statistics) on the screen. It will increase the computer time. See documentation for details -- MANDATORY --\n')
        ...
        file.write('# Number of available processors to run the simulations in parallel. See documentation for details -- MANDATORY --\n')
        file.write('*NumberOfProcessors=' + Numb_proce.get() + '\n\n')
        file.write('# Save simulated data. It is recommended do not save the simulated data because it requires a lot of space -- MANDATORY --\n')
        ...
        file.write('# Perform the coalescent history of the alignment. Coal: coalescent simulation, Phylo: user-specified phylogenetic tree. See documentation for details -- MANDATORY --\n')
        if v2.get() == 0:
            coal = 'Coal'
        elif v2.get() == 1:
            coal = 'Phylo'
        ...
        file.write('|\n')
        file.write('|# Amino acid substitution rate: fix, uniform, gamma, beta, normal, exponential. See documentation for details -- MANDATORY IF COALESCENT --\n')
        ...
        file.write('|"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""\n')
        file.write('|# User-specified phylogenetic tree/s. See documentation for details -- MANDATORY IF PHYLOGENETIC --\n')
        file.write('|*Tree=' + str(Tree_directory) + '\n')
        file.write('|\n')
        file.write('------------------------------------------------------------------------------------\n')
        file.write('				SUBSTITUTION MODEL\n')
        file.write('------------------------------------------------------------------------------------\n')
        file.write('# Model of amino acid substitution. Select at least one structurally constrained substitution (SCS) model (Fitness or Neutral) and the desired empirical substitution model (i.e., Blosum62, CpRev, Dayhoff, DayhoffDCMUT, HIVb, HIVw, JTT, JonesDCMUT, LG, Mtart, Mtmam, Mtrev24, RtRev, VT, WAG, UserEAAM). The empirical models should be spicified before SCS if any is selected. See documentation for details -- MANDATORY --\n')
        ...
        file.write('# Rate of heterogeneity across sites, +G: fix, uniform, gamma, beta, normal, exponential\n')
        file.write('RateHetSites=' + G_menu.get() + ' ' + G_menu_1.get() + ' ' + G_menu_2.get() + G_menu_3.get() + '\n\n')
        file.write('# Proportion of invariable sites, +I: fix, uniform, gamma, beta, normal, exponential\n')
        file.write('PropInvSites=' + I_menu.get() + ' ' + I_menu_1.get() + ' ' + I_menu_2.get() + I_menu_3.get() + '\n\n')
        file.write('#######################################\n')
        file.write('## SETTINGS FOR THE ESTIMATION PHASE ##\n')
        file.write('#######################################\n\n')
        file.write('# ABC iterations. Number of simulations to consider (Iterations <= NumberOfSimulations). See documentation for details -- MANDATORY --\n')
        ...
        file.write('# Summary statistics to use. See documentation for details -- MANDATORY --\n')
        file.write('*SummaryStatistics=' + SStadistics + '\n\n')
        file.write('# Multiple pages. PDF documents with multiple pages -- MANDATORY --\n')
    ...
    vF = IntVar()
    vN = IntVar()
    Struc_Model_F = Checkbutton(second_frame, text="Fitness", variable=vF, bg="gray90")
    ...
    Struc_Model_N.grid(row=74, column=1, sticky="W", padx=350)
    ...
    A_frequencies_menu=ttk.Combobox(second_frame, value=amino_distribution)
    A_frequencies_menu.bind("<<ComboboxSelected>>", pick_amino)
    A_frequencies_menu.current(0)
    A_frequencies_menu.grid(row=77, column=1, sticky='W', padx=150, pady=5)
    ...
    A_frequencies.grid(row=78, column=1, sticky='W', padx=150, pady=5)
    A_frequencies.insert(0, "0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05")
    A_frequencies.configure(state="disabled")
