	    ...
	    char *file, char *chain, int **res_index)
{
  ...
  *res_index=malloc(nres*sizeof(int));
  ...
  /*printf("prot %s has %d over %d chains different by sequence ",
	 prot->name, nc, nchain);
  printf("and %d over %d ordered residues\n", nres, nres1);*/
  ...
  int n_cont[nres1], CMAX=50;
  prot->contact=malloc(nres1*sizeof(short *));
  for(i=0; i<nres1; i++){
    prot->contact[i]=malloc(CMAX*sizeof(short));
    for(int j=0; j<CMAX; j++)prot->contact[i][j]=-1;
    n_cont[i]=0;
  }
  short **contact=Contact_list(*res, nres, &prot->n_cont, 'c', IJ_MIN);
  ...
  for(i=0; i<nres; i++){
    ...
    while(*j>=0 && *j<nres){
      ...
      if(kj>=0 && kj<nres1){
	prot->contact[k][n_cont[k]]=kj; n_cont[k]++;
	(*res_short)[k].ncont++; (*res_short)[kj].ncont++;
      }
    }
  }
  for(i=0; i<nres1; i++)prot->contact[i][n_cont[i]]=-1;
  ...
  *seq=malloc(nres1*sizeof(char)); char *s=*seq;
  ...
  for(i=0; i<nres; i++){
    if(r->chain!=old_chain){
      ...
      j=0; sc=seqres[ic]; n=n_seqres[ic]; ini=(*ini_seq)[iclus];
    }
