...
#Path
address<-paste("/Users/DavidFerreiro/Desktop/ProteinModelerABC/Examples/Fast-examples/Fasta-example3_Phylo/ABCOutputs", sep="")
setwd(address)
############################
...
pdf(figureName1)
try(plot(cv.modsel, names.arg=c("Dayhoff", "Fitness", "Neutral")), silent=TRUE)
try(legend(x="topright", inset = c(-0.05, -0.16), legend = c("Dayhoff", "Fitness", "Neutral"), xpd = TRUE, bty = "n", fill = c('grey25', 'grey50', 'grey75')), silent=TRUE)
invisible(dev.off())
############################
...
pdf(figureName1)
par(mfrow=c(1,3))
res.gfit.Dayhoff=suppressWarnings(gfit(target=FullRealmatrix, sumstat=FullSSmatrix[ModelsVector=="Dayhoff",], statistic=mean, nb.replicate=100))
write(paste("   -Dayhoff:",sep=""),"Results_text.txt",append=T)
capture.output(summary(res.gfit.Dayhoff), file = "Results_text.txt", append = TRUE)
plot(res.gfit.Dayhoff, main="Histogram under Dayhoff")
...
############################
SS_All <- matrix(nrow=1000)
for (i in 1:7){
  for(p in 1:3){
    SS <- FullSSmatrix[,i][(1000 * (p-1) + 1):(1000 * p)]
...
for (i in 1:7){
  for(p in 1:3){
    hist(SS_All[,(i * 3 + (-3 + p))], main=ModelsVector[1000 * p], xlab=col_names_matrixSS[i])
