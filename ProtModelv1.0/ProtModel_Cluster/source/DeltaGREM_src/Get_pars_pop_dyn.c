...
Read_parameters(char *FILE_IN,
		char *file_pdb, char chain[], char *file_seq, char **file_ali, 
		char **file_str_mut, int *N_str_mut,
	     ...
	     char *dir_out)
{
  ...
  file_pdb[0]='\0';
  r=Read_parameters(FILE_IN, file_pdb, chain, file_seq, file_ali, 
...
int Read_parameters(char *FILE_IN,
		    char *file_pdb, char chain[], char *file_seq,
		    char **file_ali, char **file_str_mut, int *N_str_mut,
		    int *MEANFIELD,  int *MF_COMP,
		    int *OPT_REG, int *SCORE_CV, float *REG,
		    char *file_str, float *TEMP, float *sU1,
		    float *sC0, float *sC1, int *REM, float *SEC_STR,
  ...
  while(fgets(string, sizeof(string), file_in)!=NULL){
	...
	*EXCHANGE='E';
	...
	printf("Valid options: FLUX (default), RATE, EXCH, MUT\n");
      ...
      char format[80];
      sscanf(string+7,"%s", format);
      if((strncmp(format, "WAG", 3)!=0)&&
	 (strncmp(format, "JTT", 3)!=0)){
	printf("WARNING, matrix %s not defined\n", format);
	printf("Valid options are WAG JTT\n");
...
void help(char *prog){
  ...
  printf("-lambda <lambda>    # Parameter for meanfield: l=N_pop*exp(-DG/T)\n");
  printf("                    # If lambda not given, optimal lambda is used\n");
  printf("-model <MODEL>      # Criterion to optimize lambda. Allowed:\n");
  printf("                    # NAT (Max Lik native model)\n");
  printf("                    # ALL (Max Lik misfolding model)\n");
  printf("                    # DG (DeltaG_ALL = target value)\n");
