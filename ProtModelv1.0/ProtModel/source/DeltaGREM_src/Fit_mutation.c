...
float Compute_P_mut(float *P_mut, float *P_cod, float **Q_cod,
		    ...
		    float *num_aa, float *num_dna);
void Limit_distr(float *Pi1, float **Q, int n, int *Stop_codon);
		 ...
		 char *name_file, int iter)
{
  ...
  if(neighbor==NULL){
    twonuc=mut_par[6];
    neighbor=Allocate_mat2_i(64,64, "codons");
		    ...
		    float *num_aa, float *num_dna)
{
  ...
  Limit_distr(P_cod, Q_cod, 64, Stop_codon);
  AA_distr(P_mut, P_cod, coded_aa);
...
void Limit_distr(float *Pi1, float **Q, int n, int *Forbidden)
{
		  ...
		  FILE *file_out)
{
  ...
  x[5]=1; // CpG
  x[6]=0.; // twonuc
  fprintf(file_out, "Mutation parameters: A T G C kCpG tt_ratio");
  if(Npar==7)fprintf(file_out, " twonuc");
  fprintf(file_out, "\n");

  int N=4, i,a, No=19, np=3, npopt=3; //L_aa-1;
  ...
  int freezed[Npar]; for(i=0; i<Npar; i++)freezed[i]=0;
  freezed[3]=1;
  for(N=4; N<=Npar; N++){
  ...
  fprintf(file_out, "Optimal mutation parameters:\n");
		      ...
		      float *num_aa, float *num_dna)
{
  ...
  for(k=0; k<KMAX; k++){
    ...
    float F_old=F;
    if(VBS){
      printf("it=%2d lik= %.3f  ", k, F); printf("Mut: ");
      for(j=0; j<NPAR; j++)printf(" %.3g", x[j]);
      printf("\n");
			 ...
			 float *num_aa, float *num_dna)
{
  ...
  for(k=0; k<KMAX; k++){
    ...
    printf("it=%2d %.3f %.2g %.3g  ", k, F, norm, ALPHA);
    printf("X: ");
  ...
  printf("it=%2d %.3f %.2g %.3g  ", k, Fopt, norm, ALPHA);
  printf("X: ");
