...
int Make_meanfield(struct MF_results *opt_res,
		   ...
		   FILE *file_out, FILE *file_summary, int type, char *name);
		 ...
		 struct REM E_wt, float Lambda, char *name_file)
{
  ...
  meanfield(P_MF_ia, &nat_opt, P_mut_a, NULL,
	    C_nat, i_sec, L, Lambda, E_wt, 0); // Only unfolding
  //nat_opt.lik=Compute_lik(P_MF_ia, L, f_reg_ia, f_msa_ia);
  Compute_score(&nat_opt, P_MF_ia, L, f_reg_ia, f_msa_ia, wi);
  ...
  Compute_score(&all_opt, P_MF_ia, L, f_reg_ia, f_msa_ia, wi);
  Print_test(name_file, L, all_opt, E_wt, "MF", 0);
  Print_fitness(name_file, L, mut_par, all_opt, E_wt, "MF");
  *MF_res=all_opt;
  return(0);
}
		   ...
		   FILE *file_out, FILE *file_summary, int type, char *name)
{
  ...
  // Meanfield
  float **P_opt_ia=Allocate_mat2_f(L, 20, "P_MF_old");
  ...
  res.conv=meanfield(P_MF_ia, &res, P_mut_a, P_MF_ia,
		      C_nat, i_sec, L, Lambda, E_wt, type);
  Compute_score(&res, P_MF_ia, L, f_reg_ia, f_msa_ia, wi);
  fprintf(file_out, "%.2f %6.1f %.4f %.3f %.2f %d\n",
	  Lambda, res.DG, res.score, res.h, res.Tf, res.conv);
		    ...
		    FILE *file_summary)
{
  ...
  // Native meanfield
  if(0){
    Make_meanfield(MF_res,P_MF_ia,P_mut_a,C_nat,i_sec, f_reg_ia, f_msa_ia, wi,
		   aa_seq, E_wt, file_out, file_summary, 0, "unfolding");
  }

  // Native+misfolding meanfield
  Make_meanfield(MF_res,P_MF_ia,P_mut_a, C_nat, i_sec, f_reg_ia,f_msa_ia,wi,
		 aa_seq, E_wt, file_out, file_summary, 1, "misfolding");

  Print_fitness(name_file, L, mut_par, *MF_res, E_wt, "MF");
  ...
  if(strcmp(MODEL, "NAT")==0){
    Copy_P(P_MF_ia, P_nat_opt_ia, L, 20); *MF_res=nat_opt;
  }else if(strcmp(MODEL, "ALL")==0){
    Copy_P(P_MF_ia, P_all_opt_ia, L, 20); *MF_res=all_opt;
  }else if(strcmp(MODEL, "DG")==0){
    Copy_P(P_MF_ia, P_DG_opt_ia, L, 20);  *MF_res=DG_opt;
  }else{
...
void Print_results(struct MF_results res, char *what, FILE *file_out)
{
  ...
  fprintf(file_out,
	  "%s\t%.4g\t%.4g\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.1f\t%.2f\t%.3f\n",
	  what, res.Lambda[0], res.Lambda[1], res.lik_MSA, res.KL_reg,
	  res.KL_mod, res.entropy, res.KL_mut, res.DG, res.Tf, res.h);
}
