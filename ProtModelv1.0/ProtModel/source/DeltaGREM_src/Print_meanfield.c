...
void Rate_Halpern(float *rate_HB,
		  float **exchange_HB_flux, float **exchange_HB_i, // output
		  float **P_MF_ia, float **exchange_emp,  // Input
		  float *f_emp, float *P_mut, int L);
...
int Print_profiles(float **P_MF_ia, char *tag, double DG_ave, float Lambda,
		   ...
		   char *nameout, int PRINT, float *wi, FILE *out)
{
  if(PRINT==0)return(0);
  ...
  fprintf(file_out, "    %c", AA_string[0]);
  for(a=1; a<20; a++)fprintf(file_out, "     %c", AA_string[a]);
		   ...
		   char *MATRIX, int PRINT_EXCH_ALL, float *wi, FILE *out)
{
  ...
  char namexc[20], EXCH=EXCHANGE; EXCH='F';
  if((EXCH!='E')&&(EXCH!='F')&&(EXCH!='M')){
  ...
  // Mutation model
  float **exchange_mut=Allocate_mat2_f(20, 20, "Exchange_mut");
  float **rate_mut=Allocate_mat2_f(20,20, "rate_mut"), P_mut[20];
  Compute_rate_matrix(rate_mut, P_mut, P_cod, Q_cod);
  ...
  if(PRINT_EXCH_ALL){
    // Print exchangeability matrices in <>_rate_mut.dat
    sprintf(name, "%s_rate_mut.dat", nameout);
    fprintf(out, "Printing Mutational and empirical (flux) exchangeability ");
    fprintf(out, "matrices in %s\n", name);
    file_mat=fopen(name, "w"); //printf("Writing %s\n", name);
    fprintf(file_mat,
	    "#ab E^mut(a,b) ba  E^mut(b,a)  E^flux(a,b)\n");
    for(a=0; a<20; a++){
      for(b=a+1; b<20; b++){
  ...
  if(PRINT_EXCH_ALL){
    ...
    if(EXCH=='M'){
      fprintf(file_mat,
	      ...
	      tt_ratio, TWONUC);
      fprintf(out, "Exchangeability: genetic code, tt_ratio=%.2f TWONUC=%.3f\n",
	      tt_ratio, TWONUC);
    }else if(EXCH=='F'){
      fprintf(file_mat,"# Exchangeability such that Mean FLUX as %s model\n",
	      MATRIX);
      fprintf(out,"Exchangeability such that Mean FLUX as %s model\n", MATRIX);
    }
    if(FORMAT==0){
      fprintf(file_mat, "AA ");
  ...
  //float Rate_abs_mut[L], Rate_abs_emp[L], Rate_abs_flux[L];
  float Rate_HB_mut[L], Rate_HB_emp[L], Rate_HB_flux[L];
  float **exchange_HB_flux=Allocate_mat2_f(20,20,"exchange_HB_flux");
  float **exchange_HB_i=NULL;
  if(PRINT_EXCH_ALL)exchange_HB_i=Allocate_mat2_f(20,20, "exchange_HB_i");
  Rate_Halpern(Rate_HB_mut, NULL, NULL,
	       P_MF_ia, exchange_mut, NULL, P_mut, L);
  Rate_Halpern(Rate_HB_emp, exchange_HB_flux, NULL,
	       P_MF_ia, exchange_emp, f_emp, P_mut_a, L);
  ...
  if(EXCH=='E'){
    ...
    strcpy(namexc, "FLUX");
    sprintf(type, "exchangeability_HB_%s_FLUX", MATRIX);
  ...
  fprintf(file_out, "# Equal       %.3f %.3f %6.3f\n",
	  Compute_rate_abs(p, exchange_flux, 20), log(20.),
	  Corr_vM(p, exchange_flux, 20));
  fprintf(file_out, "# mean-field  %.3f %.3f %6.3f\n",
	  Compute_rate_abs(F_MF, exchange_flux, 20), Entropy(F_MF, 20),
	  Corr_vM(F_MF, exchange_flux, 20));
  fprintf(file_out, "# %s_%s     %.3f %.3f %6.3f\n", MATRIX, namexc,
	  Compute_rate_abs(f_emp, exchange_flux, 20), Entropy(f_emp, 20),
  ...
  double hydro_tot=0;
  float norm=Compute_rate_abs(F_MF, exchange_HB_flux, 20);
  struct res_short *rs=res;
  int ii=1;
  for(i=0; i<L; i++){
    float *P_ia=P_MF_ia[i];
    Rate_Halpern(Rate_HB_flux+i, NULL, exchange_HB_i,
		 P_MF_ia+i, exchange_HB_flux, f_emp, P_mut_a, 1);
    if(PRINT_EXCH_ALL && wi[i]>0){
      fprintf(file_mat, "SITE %3d %c\n", ii, res[i].seq); ii++;
      Print_matrix(exchange_HB_i, iaa, norm, file_mat,
		   AA_string, P_ia, FORMAT);
    }
    ...
    fprintf(file_out, "%s %c %c %d %.3f %5.2f",
	    rs->pdbres, rs->seq, rs->sec_str, rs->ncont,
  ...
  float **exchange_ave=Allocate_mat2_f(20,20,"exchange_ave");
  for(a=0; a<20; a++){
    for(b=0; b<a; b++){
  ...
  if(PRINT_EXCH_ALL){
    ...
    Print_matrix(exchange_ave, iaa, norm, file_mat, AA_string, F_MF, FORMAT);
    if(FORMAT){
  ...
  float ncont[L]; for(i=0; i<L; i++)ncont[i]=res[i].ncont;
  float r=Corr_coeff(ncont, hydro_i, L);
  ...
  r=Corr_coeff(entropy, Rate_HB_emp, L);
  ...
  if(PRINT_EXCH_ALL){
    ...
    float R_emp_s[20], Rf_emp_s[20], Rf_emp[20], R_mut[20];
    for(a=0; a<20; a++){
      double R=0; for(b=0; b<20; b++)if(b!=a)R+=exchange_flux[a][b];
      R_emp_s[a]=R/19; R_mut[a]=-rate_mut[a][a];
  ...
  if(rate_mut)Empty_matrix_f(rate_mut, 20);
  if(exchange_mut)Empty_matrix_f(exchange_mut, 20);
  ...
  if(exchange_HB_i)Empty_matrix_f(exchange_HB_i, 20);
...
float **Empirical_exchangeability(float *f_emp, char *MATRIX)
{
  float **exch_emp=Allocate_mat2_f(20, 20,"exchange_emp");
				...
				char EXCHANGE, float **Pair_MF, float *F_MF)
{
  ...
  float **exchange_emp=Allocate_mat2_f(20,20,"exchange_emp"); int a, b;
  if(EXCHANGE=='E'){
...
{
  // Rescale exchangeability
  float **exchange_flux=Allocate_mat2_f(20,20,"exchange_flux"); int a, b;
  // Impose that average flux (F) is as in empirical model
  for(a=0; a<20; a++){
    for(b=0; b<a; b++){
...
float **Pair_matrix(float *F_MF, float **P_MF_ia, int L, float *P_mut_a,
		    int *iaa, char *name_file, char *AA_string, int FORMAT)
{
  float **Pair_MF=Allocate_mat2_f(20,20,"Pair_MF");
		  ...
		  float *f_emp, float *P_mut, int L)
{
  ...
  float P_sel[20], log_P_sel[20]; int i,a,b;
  for(i=0; i<L; i++){
    rate_HB[i]=0;
    float *Pi=P_MF_ia[i];
    for(a=0; a<20; a++){
	...
	R_a+=Pi[b]*e_fix;
	if(exchange_HB_i){
	  exchange_HB_i[a][b]=e_fix;
	  exchange_HB_i[b][a]=e_fix;
	}
	if(exchange_HB_flux)
