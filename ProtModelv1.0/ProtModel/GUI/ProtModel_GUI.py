...
def main_window():
    ...
    def save_selected_values():
        ...
        def start_ProtModel():
            ...
            call()
            Progress.start(20)
            os.system('chmod +x ProtModel.py')
            #threading.Timer(1.0, start_ProtModel).start()
            #global p
            #p = sub.Popen('./ProtModel-1.py', stdout=sub.PIPE, stderr=sub.PIPE, bufsize=0) #sin stderr
            p = sub.Popen('./ProtModel.py', stdout=sub.PIPE, bufsize=0, universal_newlines=True) #bufsize=1 sin stderr
