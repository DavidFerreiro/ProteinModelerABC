...
#Path
address<-paste("/User_route_to_the_new_directory", sep="")
setwd(address)
############################
...
try(legend(x="topright", inset = c(-0.05, -0.16), legend = c("Fitness", "HIVw", "Neutral"), xpd = TRUE, bty = "n", fill = c("grey30", "grey70", "grey90")), silent=TRUE)
invisible(dev.off())
############################
...
res.gfit.HIVw=suppressWarnings(gfit(target=FullRealmatrix, sumstat=FullSSmatrix[ModelsVector=="HIVw",], statistic=mean, nb.replicate=100))
...
SS_Abs <- matrix(nrow=3000)
Retained_Simu <- 1000*ABC_Tolerance
...
for (i in 1:7){
  for(p in 1:3){
    SS <- SS_Abs[,(i * 3 - 2):(i * 3)][(1000 * (p-1) + 1):(1000 * p),]
    SS <- SS[order(SS[,2]),]
    SS_Tol <- cbind(SS_Tol, SS[1:Retained_Simu, 1])
    SS_Tol <- cbind(SS_Tol, SS[1:Retained_Simu, 3])
  }
}
SS_Tol <- SS_Tol[,-1]
...
for (i in 1:7){
  for(p in 1:3){
    plot(y=SS_Tol[,(i * 3 * 2 + (-7 + p*2))], x=SS_Tol[,(i * 3 * 2 + (-6 + p*2))], main=ModelsVector[1000 * p], ylab=col_names_matrixSS[i], xlab="Theta", ylim=c(MinMax[i*2-1], MinMax[i*2]))
