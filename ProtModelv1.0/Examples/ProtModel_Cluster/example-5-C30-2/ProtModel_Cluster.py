...
with open('launch_Simu.sh', 'w') as ls:
    ls.write('#!/bin/bash\n')
    ls.write('#SBATCH --mem=150GB -n ' + str(Variables.NumberOfProcessors) + ' -t 08:00:00\n')
    ...
    with open('order.py', 'w') as O:
        ...
        O.write('print("PROTMODEL has finished!!\\n")\n')
        O.close()
...
with open("ABCAnalysis.r", "w") as P:
    ...
    for x in range(1, len(Models) + 1):
        P.write('for (j in ' + str((x - 1) * Variables.NumberOfSimulations + 1) + ':' + str(
            x * Variables.NumberOfSimulations) + ')\n')
        P.write('    {\n')
    ...
    P.write('write(paste("Model selection with abc - cross validation - based on ' + str(
        Variables.ABCIterations) + ' samples",sep=""),"Results_text.txt",append=T)\n')
    ...
    P.write('figureName1<-paste("./Results_ConfusionMatrix_' + str(
        Variables.ABCIterations) + 'samp_",ThisName,".pdf",sep="")\n')
    P.write('pdf(figureName1)\n')
    ...
    P.write(
        'try(legend(x="topright", inset = c(-0.05, -0.16), legend = c(' + Str_Models + '), xpd = TRUE, bty = "n", fill = c("grey30", "grey70", "grey90")), silent=TRUE)\n')
    P.write('invisible(dev.off())\n')
    P.write('############################\n\n')
    ...
    if Variables.MultiPage == 'Yes':
        ...
        for x in Models:
            P.write('res.gfit.' + str(
                ...
                x) + '",], statistic=mean, nb.replicate=' + str(Variables.ABCIterations) + '))\n')
    ...
    P.write('    SS <- FullSSmatrix[,i][(' + str(Variables.NumberOfSimulations) + ' * (p-1) + 1):(' + str(
        Variables.NumberOfSimulations) + ' * p)]\n')
    ...
    if Variables.MultiPage == 'Yes':
        if len(SS_list) == 7:
            ...
            P.write('for (i in 1:' + str(len(Variables.SummaryStatistics.split(' '))) + '){\n')
            P.write('  for(p in 1:3){\n')
            P.write('    hist(SS_All[,(i * 3 + (-3 + p))], main=ModelsVector[' + str(
                Variables.NumberOfSimulations) + ' * p], xlab=col_names_matrixSS[i])\n')
    ...
    P.write('for (i in 1:' + str(len(Variables.SummaryStatistics.split(' '))) + '){\n')
    P.write('  for(p in 1:3){\n')
    P.write('    SS <- SS_Abs[,(i * 3 - 2):(i * 3)][(' + str(Variables.NumberOfSimulations) + ' * (p-1) + 1):(' + str(
        Variables.NumberOfSimulations) + ' * p),]\n')
    ...
    if Variables.MultiPage == 'Yes':
        if len(SS_list) == 7:
            ...
            P.write('  for(p in 1:3){\n')
            P.write(
                '    plot(y=SS_Tol[,(i * 3 * 2 + (-7 + p*2))], x=SS_Tol[,(i * 3 * 2 + (-6 + p*2))], main=ModelsVector[' + str(
                    Variables.NumberOfSimulations) + ' * p], ylab=col_names_matrixSS[i], xlab="Theta", ylim=c(MinMax[i*2-1], MinMax[i*2]))\n')
            P.write('    abline(h = FullRealmatrix[,i], col = "blue", main=col_names_matrixSS[i])\n')
            P.write('  }\n')
            P.write('}\n')
        elif len(SS_list) < 7 and len(SS_list) > 4:
