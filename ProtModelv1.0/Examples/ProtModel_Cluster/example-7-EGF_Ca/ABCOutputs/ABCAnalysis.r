...
try(legend(x="topright", inset = c(-0.05, -0.16), legend = c("Blosum62", "Fitness", "Neutral"), xpd = TRUE, bty = "n", fill = c("grey30", "grey70", "grey90")), silent=TRUE)
invisible(dev.off())
############################
...
pdf(figureName1)
par(mfrow=c(1,3))
res.gfit.Blosum62=suppressWarnings(gfit(target=FullRealmatrix, sumstat=FullSSmatrix[ModelsVector=="Blosum62",], statistic=mean, nb.replicate=100))
