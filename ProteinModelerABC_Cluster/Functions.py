...
def SimulationSS(zoe):
    ...
    #else:
        #os.system('rm  Input_DeltaGREM' + str(zoe + 1) + '.in ')
...
def Cesga_Empirical_Simu(zoe):
  ...
  with open("ProteinModelerABC_arguments.txt", "r") as Empi:
...
def CheckSimuE(min, max):
    ...
    for x in range(min, max):
        if os.path.getsize(Variables.Simulations_Name[x]) == 0:
            ...
            num_b = '-:' + number
            with open("ProteinModelerABC_arguments.txt", "r") as openfile:
                for line in openfile:
                    if num_b in line:
                        with open("ProteinModelerABC_E-Repeat_arguments.txt", "a") as R_file:
                            R_file.write(line)

    if len(NumberofRepeats) != 0:
        #Ejecutar de nuevo la simiulacion
        with open('ProteinModelerABC_E-Repeat_arguments.txt', 'r') as S:
            for line in S:
...
def FitnessFile():
    ...
    with open('Pop_evol.in', 'w') as fp:
        ...
        fp.write('TEMP=	' + str(Variables.TEMP) + '			# Temperature\n')
        fp.write('S0=	' + str(Variables.S0) + '			# s0, configurational entropy per residue (unfolded)\n')
        fp.write('SC1=    ' + str(Variables.SC1) + '			# configurational entropy per residue (misfolded)\n')
        fp.write('SC0=    ' + str(Variables.SC0) + '			# configurational entropy offset (misfolded)\n')
        fp.write('REM3=	' + str(Variables.REM3) + '			# Third cumulant in REM calculation\n')
        fp.write('NEUTRAL=	0		# If 1, Neutral landscape, otherwise population size dependent selection \n')
        fp.write('NPOP=	' + str(Variables.NPOP) + '			# N_pop, population size\n')
...
def Cesga_Fitness_Simu(zoe):
  ...
  with open("ProteinModelerABC_S_arguments0.txt", "r") as Fit:
...
def CheckSimuF(min, max):
    ...
    for x in range(min, max):
        if os.path.getsize(Variables.Simulations_Name[x]) == 0:
            ...
            #Crear archivo nuevo para la simulacion. Uno para cada tipo (emp, fit, neu) de momento solo Fitness
            num_b = '-:' + number
            with open("ProteinModelerABC_S_arguments0.txt", "r") as openfile:
                for line in openfile:
                    if num_b in line:
                        with open("ProteinModelerABC_F-Repeat_arguments.txt", "a") as R_file:
                            R_file.write(line)

    if len(NumberofRepeats) != 0:
        with open('ProteinModelerABC_F-Repeat_arguments.txt', 'r') as S:
            for line in S:
                os.system("./bin/Protein_EvolverABC/ProteinEvolverProtABC1.2.0 " + line)
...
def Cesga_Fitness_SS(zoe):
    ...
    with open("ProteinModelerABC_S_arguments0.txt", "r") as Stru0:
...
def Cesga_Neutral_Simu(zoe):
  ...
  with open("ProteinModelerABC_S_arguments1.txt", "r") as Neut:
...
def CheckSimuN(min, max):
    ...
    for x in range(min, max):
        if os.path.getsize(Variables.Simulations_Name[x]) == 0:
            ...
            #Crear archivo nuevo para la simulacion. Uno para cada tipo (emp, fit, neu) de momento solo Fitness
            num_b = '-:' + number
            with open("ProteinModelerABC_S_arguments1.txt", "r") as openfile:
                for line in openfile:
                    if num_b in line:
                        with open("ProteinModelerABC_N-Repeat_arguments.txt", "a") as R_file:
                            R_file.write(line)

    if len(NumberofRepeats) != 0:
        with open('ProteinModelerABC_N-Repeat_arguments.txt', 'r') as S:
            for line in S:
                os.system("./bin/Protein_EvolverABC/ProteinEvolverProtABC1.2.0 " + line)
...
def Cesga_Neutral_SS(zoe):
    ...
    with open("ProteinModelerABC_S_arguments1.txt", "r") as Stru1:
