...
def SimulationSS(zoe):
    ...
    # time.sleep(1)
    Sequence = bin.Scripts.Variables.Simulations_Name[int(zoe)]
    SFormat = ''
    # Write a fasta file
    lines = []
...
def CheckSimuE(min, max):
    ...
    for x in range(min, max):
        if os.path.getsize(bin.Scripts.Variables.Simulations_Name[x]) == 0:
            NumberofRepeats.append(bin.Scripts.Variables.Simulations_Name[x])
            number = bin.Scripts.Variables.Simulations_Name[x][9:]
            ...
            os.system('rm -r ' + bin.Scripts.Variables.Simulations_Name[x])
