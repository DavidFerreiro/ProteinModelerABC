...
#Path
address<-paste("Route to Fast_examples/Fast_example3_ABCMethods/Outputs/ABCOutputs/mnlogistic", sep="")
setwd(address)
############################
...
pdf(figureName1)
try(plot(cv.modsel, names.arg=c("Fitness", "Neutral", "VT")), silent=TRUE)
try(legend(x="topright", inset = c(-0.05, -0.16), legend = c("Fitness", "Neutral", "VT"), xpd = TRUE, bty = "n", fill = c('grey25', 'grey50', 'grey75')), silent=TRUE)
invisible(dev.off())
############################
...
res.gfit.VT=suppressWarnings(gfit(target=FullRealmatrix, sumstat=FullSSmatrix[ModelsVector=="VT",], statistic=mean, nb.replicate=100))
write(paste("   -VT:",sep=""),"Results_text.txt",append=T)
capture.output(summary(res.gfit.VT), file = "Results_text.txt", append = TRUE)
plot(res.gfit.VT, main="Histogram under VT")
invisible(dev.off())
