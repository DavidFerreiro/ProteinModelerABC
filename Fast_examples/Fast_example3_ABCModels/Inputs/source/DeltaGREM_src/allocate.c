...
float **Allocate_mat2_f(int n1, int n2, char *name){
  int i, j;
  float **matrix=malloc(n1*sizeof(float *));
  if(matrix==NULL){
    printf("ERROR, %s (%d X %d) out of memory\n", name, n1, n2);
    exit(8);
  }
  for(i=0; i<n1; i++){
    matrix[i]=malloc(n2*sizeof(float));
    if(matrix[i]==NULL){
      printf("ERROR, %s (%d X %d) out of memory for line %d\n",
	     name, n1, n2, i); exit(8);
    }
...
int ** Allocate_mat2_i(int n1, int n2, char *name){
  int i, j;
...
double **Allocate_mat2_d(int n1, int n2, char *name){
  int i, j;
