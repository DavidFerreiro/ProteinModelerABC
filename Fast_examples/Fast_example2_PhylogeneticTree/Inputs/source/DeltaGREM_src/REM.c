...
int LEN_MAX=250; // Max. l for REM calculations
...
double Threading_energies(double *EC1, double *EC2, double *EC3, double *U2,
			  double *Nc1, double *Nc2, double *Nc3, 
			  short *aa_seq, int len_nat);
float G_misfold_threading(struct REM *E);
static int tail(int i, int L);
		      ...
		      char *file_str, int ini)
{
  // Temperature
  if(Econt_T==NULL){
    Econt_T=Allocate_mat2_f(NAA, NAA, "E_cont");
...
int Initialize_REM(char *file_str, char *nameout)
{
  ...
  double num_nc[L_max], nc_sum[L_max], nc_z_sum[L_max], nc_z2_sum[L_max];
  for(i=0; i<L_max; i++){
  ...
  for(int d=0; d<=DTAIL_MAX; d++){
	...
	yy[n]=(nc_nc[d][i]/(nc_ave[d]*nc_ave[i])); x[n]=i; n++;
      }
    }
    ...
    else{nc_nc_1[d]=0; nc_nc_0[d]=nc_nc[d][L2+n-1];}
    if(d<15 || d==DTAIL_MAX){
			 ...
			 int L)*/
{
  ...
  double x=pow(L,-0.33333333);
  float core=(1+Slope_x*x);
  float Lcore=L*core;
  float Lc2=Lcore*Lcore;

  float scale=pow(L, z_expo); // z_expo=-0.40 instead of -0.48 gives better <NC^2>
  ...
  if(L>LEN_MAX){
    ...
    for(l=LEN_MAX; l<L; l++){
      ...
      /*for(d=0; d<=DTAIL_MAX; d++){
	nc_nc_L[d][l]=(nc_nc_0[d]+nc_nc_1[d]*log_l[l]);
	nc_nc_Nc_L[d][l]=(nc_nc_z_0[d]+nc_nc_z_1[d]*log_l[l]);
	}*/
    }
  }
...
/*// Empty_memory
  ...
  C221_ij   =malloc(L*sizeof(double));
  C232_i    =malloc(L*sizeof(double));
...
{  
  ...
  //printf("DBG: Computing E_loc in DG\n");
  E->E_loc=Compute_E_loc(i_sec, aa_seq, L);
			       ...
			       int res_mut, short aa_new)
{
  ...
  float dEl=Delta_E_loc(i_sec, aa_old, aa_new);
  E->E_nat+=dEl;
...
float Compute_Tfreeze(struct REM *E){
  ...
  float K3=3*E->E3, K2=E->E2/2, S3=3*E->S_C;
				...
				short *aa_seq, int **C_nat, int *i_sec)
{
  ...
  Threading_energies(&E->E1, &E->E2, &E->E3, &U2, &Nc1, &Nc2, &Nc3,
		     aa_seq, E->L);
...
float Print_DG_contfreq(struct REM *E, char *name)
{
  ...
  float T_STEP=0.01, T_INI=E->Tf-2*T_STEP, T_END=E->Tf+50*T_STEP;
...
void Test_contfreq(struct REM *E, short *aa_seq, int **C_nat, int *i_sec,
		   char *name)
{
  ...
  double EC1, EC2, EC3, U2, E_ave, Nc1, Nc2, Nc3; int L=E->L;
  E_ave=Threading_energies(&EC1, &EC2, &EC3, &U2, &Nc1, &Nc2, &Nc3,
			   aa_seq, L);
  ...
  sprintf(s,
	  ...
	  "DG_misfold/L = %.4g\n"
	  "Boltzmann average E/L: %.4g\n",
	  E->T, E->L, E->S_U, E->S_C,
	  E->REM, 
	  Nc1/L, (Nc2/(Nc1*Nc1)-1),
	  (Nc3/(Nc1*Nc1*Nc1)-3*Nc2/(Nc1*Nc1)+2),
	  U2, EC1, EC1/L, EC2/2, EC2/(2*L),
	  EC3/6, EC3/(6*L), (EC1-EC2/(2*E->T))/L, E_ave/L);
  printf("%s", s);
  fprintf(file_out, "%s", s);

  int REM=E->REM; E->REM=3;
  Compute_DG_overT_contfreq(E, aa_seq, C_nat, i_sec, 1);
  E->REM=REM;
  printf("DG computed\n");
  ...
  sprintf(s,
	  ...
	  //"E36 used: %d\n"
	  E->E_nat/L, E->S_U/L,
	  NC1/L, zs2/(Y_ave*Y_ave), zs3/(Y_ave*Y_ave*Y_ave), //NC2/(NC1*NC1), NC3/(NC1*NC1*NC1),
	  ...
	  //E->E353, E->E363, E_36, 
	  H-E->S_C/L, H, H+E->E3/(6*E->T*E->T*L));
  printf("%s", s);
  fprintf(file_out, "%s", s);
			  ...
			  short *aa_seq, int len_pdb)
{
  ...
  *Nc1=0; *Nc2=0; *Nc3=0;
  ...
  /*printf("Selecting fragments >= %d residues with ", LEN_THR);  //< %d, NCmax
  ...
  for(short j_prot=0; j_prot< N_PROT; j_prot++){
	...
	ini+=len_dom;
      ...
      double w=exp(-e1); E_ave+=e1*w; E_norm+=w;
    } // end fragment 
  } // end protein
