...
#Path
address<-paste("Route to Fast_examples/Fast_example2_PhylogeneticTree/Outputs/ABCOutputs", sep="")
setwd(address)
############################
...
pdf(figureName1)
try(plot(cv.modsel, names.arg=c("Fitness", "LG", "Neutral")), silent=TRUE)
try(legend(x="topright", inset = c(-0.05, -0.16), legend = c("Fitness", "LG", "Neutral"), xpd = TRUE, bty = "n", fill = c('grey25', 'grey50', 'grey75')), silent=TRUE)
invisible(dev.off())
############################
...
res.gfit.LG=suppressWarnings(gfit(target=FullRealmatrix, sumstat=FullSSmatrix[ModelsVector=="LG",], statistic=mean, nb.replicate=100))
write(paste("   -LG:",sep=""),"Results_text.txt",append=T)
capture.output(summary(res.gfit.LG), file = "Results_text.txt", append = TRUE)
plot(res.gfit.LG, main="Histogram under LG")
